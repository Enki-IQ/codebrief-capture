...
2. `/codebrief-capture:codebrief-login` — run it in your terminal and paste the key at the **hidden prompt**; it's stored in your OS keychain and never enters the chat. If you're in a connected git repo, this also enables capture for that repo.
3. `/codebrief-capture:codebrief-enable` — turn on capture for a different repo (or re-enable one you disabled).
...
5. `/codebrief-capture:codebrief-status` — login state, whether the `claude` CLI is found, and which repos are enabled.
6. `/codebrief-capture:codebrief-disable` — stop capturing the current repo. `/codebrief-capture:codebrief-logout` — clear credentials.

Login/enable also add a `Codebrief: capturing` (or `off`) segment to Claude Code's status line, chained onto whatever status line you already had configured — no separate setup step.
