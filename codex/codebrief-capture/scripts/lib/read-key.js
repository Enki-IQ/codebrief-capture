...
import { stdinHasPipedKey } from "./login-mode.js";
...
export async function readLoginKey(rest, io = {}) {
  const {
    ...
    hasPipedKey = stdinHasPipedKey,
    readPiped = readPipedStdin,
    readHidden = readHiddenTty,
  } = io;
  const ki = rest.indexOf("--key");
  if (ki >= 0 && rest[ki + 1]) return rest[ki + 1].trim();
  if (hasPipedKey()) return (await readPiped()).trim();
  if (!isTTY) return "";
