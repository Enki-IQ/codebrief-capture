...
test("postIntent throws IngestError(401) when the key is invalid/revoked/expired", async () => {
  ...
  await assert.rejects(
    () => postIntent({ apiBaseUrl: "https://app.codebrief.ai", apiKey: "dead", repoFullName: "a/b", records: [{ kind: "plan" }], fetchImpl }),
...
test("postIntent throws IngestError(403) when the repo isn't connected to the key's org", async () => {
  ...
  await assert.rejects(
    () => postIntent({ apiBaseUrl: "https://app.codebrief.ai", apiKey: "ok", repoFullName: "a/b", records: [{ kind: "plan" }], fetchImpl }),
