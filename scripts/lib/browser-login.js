...
export function buildAuthorizeUrl(apiBaseUrl, port, state) {
  return `${apiBaseUrl}/cli/authorize?port=${port}&state=${encodeURIComponent(state)}`;
}
...
export async function exchangeCode({ apiBaseUrl, code, fetchImpl = fetch }) {
  const res = await fetchImpl(`${apiBaseUrl}/api/cli/exchange`, {
...
export function runLoopbackLogin({ apiBaseUrl, timeoutMs = 300_000, open = openBrowser, exchange = exchangeCode }) {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      clearTimeout(timer);
      server.close();
      if (!ok) return reject(new Error("state mismatch or missing code"));
      try { resolve(await exchange({ apiBaseUrl, code })); } catch (e) { reject(e); }
    });
    const timer = setTimeout(() => { if (!settled) { settled = true; server.close(); reject(new Error("login timed out")); } }, timeoutMs);
    server.on("error", (e) => { if (!settled) { settled = true; clearTimeout(timer); reject(e); } });
    server.listen(0, "127.0.0.1", () => {
      const { port } = server.address();
      open(buildAuthorizeUrl(apiBaseUrl, port, state));
    });
  });
}
